                ...
                : <div>
                    ...
                    {cart.map((prod) =>
                        <div key={prod.id} className=" d-flex flex-row ml-50 mr-50 border-b-2 border-gray-500">
                            ...
                            <div className="d-flex flex-row  mt-50 md:gap-200">
                                <img src={`${prod.photo}`} className="h-80 w-80 md:h-100 md:w-100"></img>
                                <div className="d-flex flex-column justify-center gap-20 ml-50">
                                    <p className="text-white font-card text-s self-center ml-10 md:text-l">{prod.name}</p>
                                ...
                                <span onClick={() => deleteFromCart(prod.id,prod.price)}className='d-flex flex-row items-center p-px hover:animate-bounce ml-50 '><FontAwesomeIcon icon={faX} style={{ color: "white", }} /></span>
                            </div>
                        </div>)}
